...
def _order_payload(cardinal: "Cardinal", order: Any, event: Any = None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    scope = {
        "orderId": str(order.id),
        "returnUrl": f"https://funpay.com/orders/{order.id}/",
        "chatId": getattr(order, "chat_id", None),
...
def _handle_message_event(cardinal: "Cardinal", event: NewMessageEvent | LastChatMessageChangedEvent) -> None:
    ...
    scope = {
        "chatId": message.chat_id,
        "returnUrl": f"https://funpay.com/chat/?node={message.chat_id}",
    }
    _enqueue("messaging.message.received", f"funpay:{cardinal.account.id}:message:{payload['messageId']}", payload, scope, MESSAGE_EVENT_VERSION)
